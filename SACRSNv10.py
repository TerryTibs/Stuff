...
# ==========================================
# 1. Configuration: Consolidated and Tuned for Stability/Convergence
# ==========================================
CONFIG = {
    "seq_len": 32,
    ...
    "ponder_penalty": 0.001,  # Slightly increased to encourage lower steps
    ...
    # --- TRAINING ---
    "epochs": 1000,           # Balanced for convergence without excessive overfitting
    "learning_rate": 0.001,   # Starting High
    "grad_clip": 0.5,         
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "eps": 1e-5               # Increased for better numerical stability
}
...
# ==========================================
# 5. The Master Model (With Temporal Buffer)
# ==========================================
class AdvancedCRSN(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [CRITICAL] Buffer for Temporal Consistency
...
def train():
    ...
    print(f"--- Training Ultimate CRSN ({CONFIG['embedding_dim']}D) ---")
    print(f"--- Features: Graph Bias | ACT | Entropy Reg | Scheduler | Temporal Consistency ---")
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(0, len(data_tensor) - 1):
                ...
                # --- Entropy Loss (Prevent Dead Symbols) ---
                probs = F.softmax(logits, dim=-1)
                log_probs = F.log_softmax(logits, dim=-1)
                entropy = -(probs * log_probs).sum()
                loss_entropy = -entropy_weight * entropy
                
                # --- Static Symbol Consistency (Graph Sharpness) ---
                ...
                # --- [FIX] Temporal Consistency (Buffer Momentum) ---
                ...
                loss = (loss_pred + 
                        ...
                        0.1 * vq_loss + 
                        loss_entropy + 
                        loss_static_consistency + 
                        loss_temporal)
                
                if torch.isnan(loss):
                    print(f"PANIC: NaN Loss at Epoch {epoch}")
                    hidden = None
                    continue
