...
CONFIG = {
    ...
    # --- ACT & STACK (The focus of this version) ---
...
class UberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, prev_sym=None):
        ...
        # Return extra diagnostic info
        return logits, z_weighted, current_sym, ponder_cost, vq_loss_total, perplexity_total, stack_depth_log
...
def train():
    ...
    print(f"--- Saving visuals to current directory ---")
...
# ==========================================
# 8. Visualization Suite (The Scientific Output)
# ==========================================
def generate_diagnostics(model):
    ...
    plt.title(f"1_topology (Active: {len(G.nodes)})")
    plt.savefig("1_topology.png")
    print("Saved 1_topology.png")
    plt.close()
    ...
    plt.title("2_stack_mri (Memory Depth over Time)")
    plt.xlabel("Time Step")
    plt.ylabel("Stack Pointer")
    ...
    plt.title("3_act_profile (Thinking Steps per Token)")
    plt.xlabel("Token")
...
if __name__ == "__main__":
    ...
    generate_diagnostics(trained_model)
    
    try:
        from google.colab import files
        files.download(FILENAME)
        files.download("1_topology.png")
