...
# ==========================================
# 1. Configuration: The "Goldilocks" Setup
# ==========================================
CONFIG = {
    "seq_len": 32,
    ...
    "n_symbols": 64,          # Matches vocab size (no dead islands)
...
def train():
    ...
    print(f"--- Training Master CRSN ({CONFIG['embedding_dim']}D / {CONFIG['n_symbols']} Sym) ---")
...
def visualize_brain(model):
    ...
    for i in range(n_sym):
        for j in range(n_sym):
            ...
            # Threshold 0.4 allows us to see structure even if faint
            if weight > 0.4: 
                G.add_edge(f"S{i}", f"S{j}", weight=weight)
    
    # 2. Filter Dead Nodes (Smart Visualization)
    ...
    if active_nodes > 0:
        ...
        plt.title(f"Learned Logical Topology")
        plt.axis('off')
        plt.show()
    else:
...
# ==========================================
# 8. Rule Extractor (From v3)
# ==========================================
def extract_logic_rules(model, data_tensor):
    print("\n--- Extracting Explicit Logic Rules (From v3) ---")
    model.eval()
    
    # Store transitions: rules[(prev, current)] = [confidence_scores]
    rule_book = defaultdict(list)
    
    # Run through the data one last time to log the thinking
    hidden = None
    prev_sym = None
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            ...
            # Record the transition if we have a previous symbol
            if prev_sym is not None:
                # Convert tensors to python integers
                src = prev_sym.item()
                dst = sym_idx.item()
                
                # Record the "Mental Effort" required for this step
    ...
    for (src, dst), ponders in sorted_rules:
        ...
        # Only show significant rules (happened more than once)
        if count > 1:
...
if __name__ == "__main__":
    FILENAME = "crsn_master_model.pth"
    ...
    # 4. Extract Rules (The v3 Feature)
