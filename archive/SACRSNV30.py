# ============================================================
# SACRSN v30: THE AWAKENING EDITION
# Goal: Fix Codebook Collapse (PPX=1.0) using Diversity Loss
...
CONFIG = {
    ...
    # Topology & Stability
    "commitment_cost": 0.01,        # [FIX] Very low to encourage exploration
    "graph_bias_scale": 0.8,        # [FIX] High to force topological learning
    "symbol_consistency_weight": 0.01,
    "ethical_weight": 0.005,
    "diversity_weight": 0.5,        # [NEW] Force codebook usage
...
class GraphMemoryVQ(nn.Module):
    def __init__(self, latent_dim, n_symbols):
        ...
        # [FIX] Uniform Initialization (prevents clustering at 0)
...
def train():
    ...
    print(f"--- Training SACRSN v30 (The Awakening) ---")
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(len(data_tensor) - 1):
                ...
                # [NEW] Diversity Loss (Forces high PPX)
                # Minimize sum(p * log p) = Maximize Entropy
                # If model always picks same symbol, this is 0. If uniform, it's negative large.
                # We want negative large.
                avg_usage = curr_onehot # In batch 1, this is just 1 hot
                # To make this work for sequential batch=1, we use the buffer history
                ...
                loss = loss_pred + loss_ponder + 0.1*vq_loss + loss_entropy + loss_static + loss_temporal + loss_ethics + loss_diversity
