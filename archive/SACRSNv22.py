# ============================================================
# UBER-SACRSN v19: STABILIZED OMNI-SCIENTIFIC EDITION
# Fixes: Autograd safety, Persistent Stack, Phase Reconstruction
# ============================================================
...
class UberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, prev_sym=None):
        ...
        if hidden is None:
            ...
            # Unpack hidden state (Recurrence + Stack)
        ...
        # [FIX] Return tuple including Stack State for next token
        new_hidden = (z_weighted, stack_mem, stack_ptr)
        
        return logits, new_hidden, current_sym, ponder_cost, vq_loss_total, perplexity_total/act_step, ethical_loss_total, stack_history
...
def train():
    ...
    print(f"--- Training SACRSN Omni-Scientific ---")
    print(f"--- Features: Stack Persistence | Safe Autograd | Graph Bias | Consistency ---")
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(0, len(data_tensor) - 1):
                ...
                # [FIX] Detach persistent state to prevent infinite graph growth
                z_h, s_mem, s_ptr = hidden
                hidden = (z_h.detach(), s_mem.detach(), s_ptr.detach())
                ...
                # Safe In-Place update outside the graph
                with torch.no_grad():
...
def visualize_all(model):
    ...
    node_labels = {}
    for sym_idx, char_list in symbol_to_char.items():
        if char_list:
    ...
    for i in range(CONFIG["n_symbols"]):
        for j in range(CONFIG["n_symbols"]):
            if adj_probs[i, j] > 0.4: 
                G.add_edge(i, j, weight=adj_probs[i, j])
    
    isolates = list(nx.isolates(G))
    G.remove_nodes_from(isolates)
    
    plt.figure(figsize=(10, 10))
    try: pos = nx.circular_layout(G)
    except: pos = nx.spring_layout(G)
    
    nx.draw_networkx_nodes(G, pos, node_color='#a0cbe2', node_size=800, alpha=0.9)
    active_labels = {k: v for k, v in node_labels.items() if k in G.nodes}
    nx.draw_networkx_labels(G, pos, labels=active_labels, font_size=10, font_weight="bold")
    nx.draw_networkx_edges(G, pos, width=1.5, edge_color='gray', arrowstyle='->', arrowsize=15)
    
    plt.title(f"1_semantic_topology (Active: {len(G.nodes)})")
    ...
    for _ in range(200):
        with torch.no_grad():
            ...
            # Stack Depth
            if s_logs:
                # s_logs is a list of tensors from recursion
                # Need to stack them on CPU to avoid device errors
            ...
            # Unpack z from hidden for phase plot
            z = hidden[0].cpu().squeeze() # hidden is (z, mem, ptr)
...
def dream_mode(model):
    ...
    for _ in range(100):
        ...
        # [FIX] Direct Codebook Access
        z_flat = model.vq_layer.codebook[next_sym].unsqueeze(0)
        # Decoder expects flat [Real, Imag], codebook is already flat
