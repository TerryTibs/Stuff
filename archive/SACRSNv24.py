# ============================================================
# UBER-SACRSN v20: X-RAY SCIENTIFIC EDITION
# Integrates: Stack, ACT, Topology, Ethics, High-Density Viz & Logic Extraction
# ============================================================
...
# ==========================================
# 1. Configuration: "The Scientific Sweet Spot"
# ==========================================
CONFIG = {
    "seq_len": 32,
    ...
    "symbol_consistency_weight": 0.01, # Sharpens the graph
    "ethical_weight": 0.1,             # Penalizes logic breaking
...
# ==========================================
# 4. Memory Modules: Stack & Graph VQ
# ==========================================
...
class GraphMemoryVQ(nn.Module):
    ...
    def forward(self, z, prev_symbol_idx=None):
        z_flat = torch.cat([z.real, z.imag], dim=-1)
        # Safer manual distance calculation to avoid cdist shape issues
        ...
        # Calculate Perplexity (Scientific Metric for Usage)
...
# ==========================================
# 5. Core Processor & Ethical Layer
# ==========================================
class EthicalConstraint(nn.Module):
    ...
    def forward(self, prev_sym, curr_sym, adjacency):
        ...
        transitions = torch.sigmoid(adjacency[prev_sym, curr_sym])
        return torch.mean(1.0 - transitions)
...
class UberCRSN(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [FIX] Buffer for Temporal Consistency
    ...
    def forward(self, input_ids, hidden=None, prev_sym=None):
        ...
        for t in range(CONFIG["max_recursion_depth"]):
            ...
            if CONFIG["use_stack"]:
                # Pass PERSISTENT stack state into stack module
                stack_read, stack_mem, stack_ptr = self.stack(z_proc, stack_mem, stack_ptr, stack_ctrl)
                z_combined = z_proc + stack_read
                
                # Diagnostics
                depth_idx = torch.argmax(stack_ptr, dim=1).float().mean()
                stack_history.append(depth_idx)
            else:
                z_combined = z_proc
        ...
        # [FIX] Return Tuple Hidden State (Latent, StackMem, StackPtr)
        next_hidden = (z_weighted, stack_mem, stack_ptr)
        
        return logits, next_hidden, current_sym, ponder_cost, vq_loss_total, perplexity_total/act_step, ethical_loss_total, stack_history
...
def train():
    ...
    print(f"--- Training SACRSN v20 (X-Ray) ---")
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(0, len(data_tensor) - 1):
                ...
                # [FIX] Safely detach the tuple components for TBPTT
                ...
                # [FIX] Autograd Safe Temporal Consistency
                ...
                # [FIX] In-place update wrapped in no_grad
                with torch.no_grad():
...
# ==========================================
# 8. Visualization Suite (X-RAY MODE)
# ==========================================
def visualize_all(model):
    print("\n--- Generating Diagnostics & Images ---")
    model.eval()
    
    # --- 1. SEMANTIC MAPPING (Labeling Nodes) ---
    ...
    # Draw edges with dynamic width/alpha
    for (u, v), w in zip(edges, weights):
        nx.draw_networkx_edges(G, pos, edgelist=[(u, v)], 
                               width=w * 2.0, 
                               alpha=max(0.1, w), # Opacity = strength
                               edge_color='gray', 
                               arrowstyle='->', arrowsize=10)
    
    plt.title(f"1_semantic_topology (X-Ray Mode - Active: {len(symbol_to_char)}/{CONFIG['n_symbols']})")
    ...
    for _ in range(200):
        with torch.no_grad():
            logits, hidden, prev_sym, ponder, _, _, _, s_logs = model(x, hidden, prev_sym)
            
            if s_logs:
                # Need to handle tensors on CPU/GPU
                avg_depth = torch.stack([s.detach() for s in s_logs]).mean().item()
                stack_history.append(avg_depth)
            else:
...
# ==========================================
# 9. Advanced Interaction (System 2 Features)
# ==========================================
def dream_mode(model):
    ...
    for _ in range(100):
        probs = adj[curr_sym]
        probs[probs < 0.2] = 0 # Lower threshold for dreaming
        
        if probs.sum() == 0: break
        probs = probs / probs.sum()
        next_sym = np.random.choice(len(probs), p=probs)
        
        # Correctly Decode Flat Codebook
